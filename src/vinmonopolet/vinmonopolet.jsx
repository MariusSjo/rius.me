  ...
  return (
    <div>
      <div id="searchAlco">
        ...
        />
        <p id="APK_info">
          *APK: Alkohol per krone. Dette er en måleenhet som forklarer hvor mye
          alkohol man får for pengene. For å gi et perspektiv til forholdstallet
          så vil en 6-pack med øl på 4,7% til kr 200 ha 0,0705 APK.
        </p>
        <div id="alternatives">
          <select onChange={(e) => setAlternative(e.target.value)}>
            {alternatives.map((alt) => {
              return <option value={alt.value}>{alt.name}</option>;
            })}
          </select>
        </div>
      </div>
      <div id="Poloversikt">
        ...
        {liste.map((drinks) => {
          if (
            ...
            (drinks.classification.mainProductTypeId === alternative ||
              alternative === 0)
          ) {
            return (
              <div className="enheter" key={drinks.basic.productId}>
                <div>
                ...
                <div>
                  ...
                  <br />
                  Prosent: {drinks.basic.alcoholContent}% <br />
                  Volum: {drinks.basic.volume} <br />
                  APK:{" "}
                  {Math.round(
                    ((drinks.basic.volume * drinks.basic.alcoholContent) /
